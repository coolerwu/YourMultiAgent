---
name: dev-standards
description: YourMultiAgent 项目开发规范速查。当新增文件、新增模块、修改代码、重构、设计 API，或用户问“怎么写”“放哪里”“命名规范”时使用。涵盖 COLA 分层约束、后端 Python 规范、前端 React 规范、命名约定、测试规范，以及代码改动后的验证流程。
---
...
- 函数签名必须有类型注解
- `domain/` 内禁止出现 `httpx`、`sqlalchemy` 等基础设施依赖
