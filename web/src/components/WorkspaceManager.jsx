...
export default function WorkspaceManager({ open, workspace, onClose, onSaved }) {
  const [form] = Form.useForm()
  const [saving, setSaving] = useState(false)
  const isEdit = !!workspace
  const currentName = Form.useWatch('name', form)
  const currentWorkDir = Form.useWatch('work_dir', form)
  const previewDirName = workspace?.dir_name || sanitizeWorkspaceName(currentName)
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      message.success(isEdit ? '已更新' : '创建成功')
      onSaved?.(saved)
      onClose?.()
    } catch (e) {
  ...
  return (
    <Modal
      title={isEdit ? '编辑 Workspace' : '新建 Workspace'}
      open={open}
      onOk={handleOk}
      onCancel={onClose}
    ...
    >
      <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
        ...
        <Form.Item
          name="work_dir"
          label="根目录（work_dir）"
          extra="选填。留空时默认使用 DATA_DIR/workspaces/<workspace_name>；开发模式通常是当前项目 data 目录。"
        >
